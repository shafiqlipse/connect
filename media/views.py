...
def journalist(request,id):
    journalist = Media.objects.get(id=id)
    context={"journalist":journalist}
    return render(request, "media/journalist.html",context)
...
def media_profile(request):
    user=request.user
    journ = Media.objects.get(user_id=user.id)
    context={"journ":journ}
...
def inactive_journalists(request):
    journalists = Media.objects.filter(status="Inactive")
    return render(request, "media/journalists.html", {"journalists": journalists})  